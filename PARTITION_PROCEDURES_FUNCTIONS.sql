CREATE OR REPLACE FUNCTION KOKO1.GET_YEAR_MONTH_DAY (P_DATE IN DATE) 
    RETURN VARCHAR2 
  IS
  BEGIN
...
CREATE OR REPLACE FUNCTION KOKO1.ADD_PARTITION(P_TABLE_NAME IN VARCHAR2, P_DATE_YYYYMMDD IN VARCHAR2) 
    RETURN VARCHAR2
  IS
    V_CNT              INTEGER;
...
CREATE OR REPLACE PROCEDURE KOKO1.ADD_PARTITION_IF_NOT_EXISTS(P_TABLE_NAME IN VARCHAR2, P_DATE IN DATE) 
  IS
...
BEGIN
 select to_date(to_char(MAX(to_date(actual_date,'YYYY-MM-DD')),'YYYYMMDD'),'YYYYMMDD') 
   INTO v_Date1 FROM (select distinct actual_date from KOKO1);
      IF P_DATE IS NULL THEN
      RETURN NULL;
