CREATE OR REPLACE FUNCTION cc.FIRST_PAYMENT_DATE (
	...
	p_INTREST_PERIOD_FREQUENCY IN T_DM_ABS_POSITION_A.INTEREST_PERIOD_FREQUENCY%TYPE
)
RETURN DATE
IS 
v_INTV_TYPE NUMBER;
...
BEGIN
	v_INTV_TYPE := INTERVAL_TYPE(p_INTREST_PERIOD_FREQUENCY);

	IF p_LEGAL_MATURITY_DATE >= p_ACTUAL_DATE THEN
	
	select floor(months_between(p_LEGAL_MATURITY_DATE, p_ACTUAL_DATE))*(-1) 
	INTO v_MON_NUMBER from DUAL;
	
		IF ADD_MONTHS(p_LEGAL_MATURITY_DATE,ROUND(v_MON_NUMBER/v_INTV_TYPE)*v_INTV_TYPE) < p_ACTUAL_DATE THEN
		v_F_PAYMENT_DATE := ADD_MONTHS(p_LEGAL_MATURITY_DATE,
		ROUND((v_MON_NUMBER+1)/v_INTV_TYPE)*v_INTV_TYPE);
		ELSE 
	v_F_PAYMENT_DATE := ADD_MONTHS(p_LEGAL_MATURITY_DATE,ROUND(v_MON_NUMBER/v_INTV_TYPE)*v_INTV_TYPE);
		END IF;
	ELSIF p_LEGAL_MATURITY_DATE IS NULL AND p_EXPECTED_MATURITY_DATE >= p_ACTUAL_DATE THEN
		select floor(months_between(p_EXPECTED_MATURITY_DATE, p_ACTUAL_DATE))*(-1) 
		INTO v_MON_NUMBER from DUAL;
			IF ADD_MONTHS(p_EXPECTED_MATURITY_DATE,ROUND(v_MON_NUMBER/v_INTV_TYPE)*v_INTV_TYPE) < p_ACTUAL_DATE THEN
			v_F_PAYMENT_DATE := ADD_MONTHS(p_EXPECTED_MATURITY_DATE,ROUND((v_MON_NUMBER+1)/v_INTV_TYPE)*v_INTV_TYPE);
			ELSE 
			v_F_PAYMENT_DATE := ADD_MONTHS(p_EXPECTED_MATURITY_DATE,ROUND(v_MON_NUMBER/v_INTV_TYPE)*v_INTV_TYPE);
			END IF;
